...
if __name__ == "__main__":
    ...
    vis = Logger.VisConfigurator(currentTime = currentTime, visName = 'Test')
    # Initialize the logger.
    logger = Logger.LogConfigurator(logDir = Cfg.logDir, filename = currentTime)
    # Log the hyperparameters.
    logger.info('\n' + Handler.Displayer(Cfg))
    # Get the data.
    trainSet, devSet = Preprocessor.MNIST(root = Cfg.dataDir, batchSize = Cfg.batchSize)
    # Create the model.
    model = Model.Model()
    ...
    optimizer = optim.Adam(model.parameters(), lr = Cfg.learningRate)
    # Train the model.
    ...
    Logger.VisSaver(vis, visName = 'Test')